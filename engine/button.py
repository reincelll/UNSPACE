...
class button:
    ...
    def draw(self):
        ...
        if self.selected:
            color = rl.WHITE
            rl.draw_rectangle_gradient_h(self.x - offset, self.y - offset, self.width + (offset * 2), self.height + (offset * 2), rl.Color(240, 41, 55, 172), rl.Color(0, 0, 0, 0))
            if rl.is_mouse_button_down(rl.MOUSE_BUTTON_LEFT):
                offset = 2
            else:
                offset = 5
        else:
            rl.draw_rectangle_gradient_h(self.x - offset, self.y - offset, self.width / 2 + (offset * 2), self.height + (offset * 2), rl.Color(240, 41, 55, 60), rl.Color(0, 0, 0, 0))
